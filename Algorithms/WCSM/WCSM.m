function [imgL_d,imgR_d] = WCSM(I1, I2, disparity_range)
...
imgL_d = single(zeros(rows,cols));
imgR_d = single(zeros(rows,cols));
for d = disparity_range(1):disparity_range(2)
    ...
    C1crop = C1min(:,range1);
    C2crop = C2min(:,range2);
    D1crop = imgL_d(:,range1);
    D2crop = imgR_d(:,range2);
    ...
    C1min(:,range1) = C1crop;
    C2min(:,range2) = C2crop;
    imgL_d(:,range1) = D1crop;
    imgR_d(:,range2) = D2crop;
