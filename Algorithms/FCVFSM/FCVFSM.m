function [ imgL_d,imgR_d ] = FCVFSM( imgLeft,imgRight,searchrange)
numDisp = searchrange(2);
imgLeft=double(imgLeft)/ 255;
imgRight=double(imgRight)/ 255;

imgL_d=zeros(size(imgLeft,1),size(imgLeft,2));
imgR_d=imgL_d;
%
...
% for testimage = 1:4
[ imgL_d,imgR_d ] =example_referenceForCVPR11(imgLeft,imgRight,numDisp,r,eps,thresColor,thresGrad,gamma,threshBorder,gamma_c,gamma_d,r_median,UseParallelToolbox);
% end
end