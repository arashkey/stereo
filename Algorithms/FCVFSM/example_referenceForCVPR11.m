function  [ imgL_d,imgR_d ] = example_referenceForCVPR11(Il,Ir,numDisp,r,eps,thresColor,thresGrad,gamma,threshBorder,gamma_c,gamma_d,r_median,UseParallelToolbox)
tic
...
if (UseParallelToolbox)
    parfor d=1:numDisp % use parallel for loop when using the parallel computing toolbox
        ...
        dispVol(:,:,d) = q;
        dispVol1(:,:,d) = flip(q1,2);
    end
else
    parfor d=1:numDisp % use regular for loop when not using the parallel computing toolbox
        p = dispVol(:,:,d);
        p1 = dispVol1(:,:,d);
...
imgL_d = fillPixelsReference(Il, inputLabels, gamma_c, gamma_d, r_median, numDisp);

% Show final disparity map
M=strcat('Result in :' , num2str(toc) , ' for  FCVFSM');
display(M);
imgR_d=zeros(size(imgL_d));
% fileID = fopen('changing.txt','a');
% fmt = [M '\n'];
% fprintf(fileID,fmt);
% fclose(fileID);

% dlmwrite(strcat('Result_',num2str(testimage),'.txt'),final_labels)

%  imwrite((255-final_labels*4)/255,strcat('Result_',num2str(testimage),'.png'))
