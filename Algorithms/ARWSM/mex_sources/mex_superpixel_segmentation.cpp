...
void mexFunction( int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[] ){
    ...
    int rows = size[0];
    int cols = size[1];    
    double *superpixel_size = (double *)mxGetData(prhs[1]);
    double *spatial_weight = (double *)mxGetData(prhs[2]);
    
    plhs[0] = mxCreateNumericMatrix(rows, cols, mxDOUBLE_CLASS, mxREAL);
	double *result = (double *)mxGetData( plhs[0] );    
    
    int m_spcount = superpixel_size[0]; //35
//     int m_spcount = (rows*cols/(superpixel_size[0]*superpixel_size[0]) + 0.5); //35
    double m_compactness = spatial_weight[0];
    int width = cols;
	int height = rows;
    int sz = width*height;    
    ...
    const double&               compactness)
{
	...
	//klabels.resize( sz, -1 );
	//--------------------------------------------------
	//klabels = new int[sz];	//memory leak
